...
"""
def calculate_yields(fromDate, toDate, upper=20, lower=20, duration=181):
    ...
    for i in range(num_company):
        ...
        for key, value in stocks[i].items():
            ...
            for idx in range(len(buy_report)):
                delta = (0.99685 * sell_report[idx][1] - 1.00015 * buy_report[idx][1]) / buy_report[idx][1]
                strat_yield_with_fee = strat_yield_with_fee * (1 + delta)
            print(strat_yield_with_fee)
