...
class DBUpdater:
    ...
    def read_krx_code(self, fromKRX=True):
        if fromKRX:
            ...
            krx.code_list = krx.code_list.map('{:06d}'.format)
        else:
            """Naver로부터 코드 및 회사명 읽어와서 데이터프레임으로 변환"""
            code = []
            company = []
                    ...
                    with urlopen(page_url) as doc:
                        ...
                        for i in range(len(html_partial)):
                            ...
                            if not x.isdigit():
                                continue
                            code.append(int(x))
                            company.append(temp.a.get_text())
            krx = pd.DataFrame({'code': code, 'company': company})
    ...
    def update_comp_info(self):
        ...
        with self.conn.cursor() as curs:
            ...
            if rs[0] == None or rs[0].strftime('%Y-%m-%d') < today:
                krx = self.read_krx_code(fromKRX=False)
